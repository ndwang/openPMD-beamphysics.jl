...
"""
    plt_histogram(a, weights=nothing, bins=40)

This produces the same plot as Plots.histogram

For reference only.

Note that bins='auto', etc cannot be used if there are weights.
"""
function plt_histogram(a, weights=nothing, bins=40)
    cnts, bins = histogram(a, weights=weights, bins=bins)
    bar(bins[1:end-1] .+ diff(bins)./2, cnts, width=diff(bins))
end

"""
    slice_plot(particle_group, keys...; n_slice=40, slice_key=nothing, xlim=nothing, ylim=nothing, tex=true, nice=true, kwargs...)
...
- `nice`: Scale to nice units (default: true)

# Returns
- `p`: Plots.Plot object
"""
function slice_plot(particle_group, keys...; n_slice=40, slice_key=nothing, xlim=nothing, ylim=nothing, tex=true, nice=true, kwargs...)
    ...
    slice_dat = slice_statistics(particle_group, keys..., n_slice=n_slice, slice_key=slice_key)
    ...
    ux = p1 * string(particle_group.units(slice_key))
    ...
    _, f2, p2, ymin, ymax = plottable_array([ymin, ymax], nice=nice, lim=ylim)
    uy = p2 * uy
    ...
    y2, _, prey2, _, _ = plottable_array(slice_dat[y2_key], nice=nice, lim=nothing)

    # Convert to Amps if possible
    y2_units = "C/" * string(particle_group.units(x_key))
    if y2_units == "C/s"
        y2_units = "A"
    end
    ...
    p2 = twinx(p)
    ylabel!(p2, labely2)
    area!(p2, x, y2, fillalpha=0.2, color=:black)
    ylims!(p2, (0, nothing))

    # Set plot limits
    if !isnothing(xlim)
        xlims!(p, (xmin/f1, xmax/f1))
    end
    if !isnothing(ylim)
        ylims!(p, (ymin/f2, ymax/f2))
    end
...
"""
    density_plot(particle_group, key="x"; bins=nothing, xlim=nothing, tex=true, nice=true, kwargs...)
...
- `p`: Plots.Plot object
"""
function density_plot(particle_group, key="x"; bins=nothing, xlim=nothing, tex=true, nice=true, kwargs...)
    if isnothing(bins)
    ...
    # Create plot
    p = bar(hist_x, hist_y, width=hist_width, color=:grey; kwargs...)
...
"""
    marginal_plot(particle_group, key1="t", key2="p"; bins=nothing, xlim=nothing, ylim=nothing, tex=true, nice=true, ellipse=false, kwargs...)
...
- `p`: Plots.Plot object
"""
function marginal_plot(particle_group, key1="t", key2="p"; bins=nothing, xlim=nothing, ylim=nothing, tex=true, nice=true, ellipse=false, kwargs...)
    if isnothing(bins)
        n = length(particle_group)
        bins = Int(sqrt(n/4))
    end
    ...
    if length(x) == 1
        ...
        histogram2d!(p, x, y, weights=w, bins=bins, color=CMAP0, vmin=1e-20)
    end
    ...
    p_top = bar(hist_x_centers, hist_x_y, width=hist_x_width, color=:grey)
    if u1 == "s"
    ...
    p_side = barh(hist_y_centers, hist_y_y, height=hist_y_width, color=:grey)
    ...
    if !isnothing(xlim)
        xlims!(p, (xmin/f1, xmax/f1))
        xlims!(p_top, (xmin/f1, xmax/f1))
    end

    if !isnothing(ylim)
        ylims!(p, (ymin/f2, ymax/f2))
        ylims!(p_side, (ymin/f2, ymax/f2))
    end

    # Combine plots
    layout = @layout [a{0.3h} _; _ b{0.3w}]
...
function density_and_slice_plot(particle_group, key1="t", key2="p"; stat_keys=["norm_emit_x", "norm_emit_y"], bins=100, n_slice=30, tex=true)
    ...
    heatmap!(p, H, color=CMAP0, vmin=1e-16)
    ...
    max2 = maximum(peak2peak(slice_dat[k]) for k in stat_keys)
    ...
    area!(p2, x2, y2, fillalpha=0.1, color=:black)
...
function plot_fieldmesh_cylindrical_2d(fm; component=nothing, time=nothing, aspect="auto", cmap=nothing, kwargs...)
    ...
    xlabel!(p, "z (m)")
    ylabel!(p, "r (m)")
    title!(p, "$component ($(unit.unitSymbol))")
...
"""
    plot_fieldmesh_rectangular_2d(fm; component=nothing, time=nothing, aspect="auto", cmap=nothing, nice=true, kwargs...)
...
- `p`: Plots.Plot object
"""
function plot_fieldmesh_rectangular_2d(fm; component=nothing, time=nothing, aspect="auto", cmap=nothing, nice=true, kwargs...)
    ...
    llabel = "$(component[1])_$(component[2])($plane) ($prefix$(unit.unitSymbol))"
